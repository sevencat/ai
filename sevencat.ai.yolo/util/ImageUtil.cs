...
public class ImageUtil
{
	...
	public static void NormalizerPixelsToTensor(Image<Rgb24> image, MemoryTensor<float> tensor, Vector<int> padding)
	{
		...
		if (image.Height + (padding.Y * 2) != tensor.Dimensions[2] &&
		    image.Width + (padding.X * 2) != tensor.Dimensions[3])
		{
	...
	private static void WritePixel(Span<float> target, int index, Rgb24 pixel, int strideBatchR, int strideBatchG,
		int strideBatchB)
	{
