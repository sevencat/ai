...
public class Sam3Infer
{
	...
	private readonly SamConfiguration _config;
	
	//目前是写死的
	const int input_image_width_ = 1008;
	const int input_image_height_ = 1008;

	public Sam3Infer(SamConfiguration samConfiguration)
	{
		_config = samConfiguration;
		_vision_encoder = CreateInferSession(_config.VisionEncoderModelPath);
		_text_encoder = CreateInferSession(_config.TextEncoderModelPath);
		//这个暂未使用
		//_geometry_encoder=CreateInferSession(_config.);
		_decoder = CreateInferSession(_config.DecoderEncoderModelPath);
		_tokenizer = new Tokenizer(_config.TokenizerPath);
	}
	...
	public InferenceSession CreateInferSession(string model)
	{
		...
		if (_config.UseCuda)
		{
			var opt = SessionOptions.MakeSessionOptionWithCudaProvider(_config.CudaDeviceId);
